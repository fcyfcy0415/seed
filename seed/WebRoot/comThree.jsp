...
<body>
  ...
  <div class="text2">  
     <!-- 查询begin -->
      <form action="Comtwo" method="post" class="elegant-aero">
		<h1 class="h1">已发布招聘信息
    ...
    <!-- 查询end -->
    <!-- 修改招聘信息begin -->
 	<form action="ComFour" method="post" class="elegant-aero">
		<h1 class="h1">招聘信息
        ...
        <label class="label">
			...
			<select class="select" name="time">
			  <option value="<% out.print(job_Info.getJWorkExp()); %>"><% out.print(job_Info.getJWorkExp()); %></option>
		...
		<label class="label">
			...
			<select class="select" name="educational">
			  <option value="<% out.print(job_Info.getJEduRequire()); %>"><% out.print(job_Info.getJEduRequire()); %></option>
			  <option value="无要求">无要求</option>
		...
		<label class="label">
			...
			<input class="input[type=text]" id="age" type="text" name="age" 
			placeholder="输入招聘职位对年龄的要求" value="<% out.print(job_Info.getJAgeRequire()); %>"/>
   		</label>
   		<label class="label">
			<span class="label>span">语言要求</span>
			<input class="input[type=text]" id="language" type="text" name="language" 
			placeholder="输入招聘职位对语言的要求" value="<% out.print(job_Info.getJLanRequire()); %>"/>
   		</label>
		...
		<label class="label">
			...
			<input class="input[type=text]" id="position" type="text" name="position" 
			placeholder="输入您的职位类别" value="<% out.print(job_Info.getJJobType()); %>"/>
		</label>
		
		<label class="label">
			<span class="label>span">工资架构：</span>
			<input class="input[type=text]" id="wages"    type="text" name="wages" 
			placeholder="输入您的工资结构" value="<% out.print(job_Info.getJSalaryStru()); %>"/>
		</label>
		<label class="label">
			<span class="label>span">岗位职责：</span>
			<textarea class="textarea" id="duties"                    name="duties" 
			placeholder="岗位职责"  ><% out.print(job_Info.getJPositionState()); %></textarea>
		</label>
		<label class="label">
			<span class="label>span">岗位要求：</span>
			<textarea class="textarea" id="requirements"              name="requirements" 
			placeholder="输入岗位要求"   ><% out.print(job_Info.getJJobRequire()); %></textarea>
		</label>
		<h4 class="h4">
		...
		<label class="label">
			...
			<textarea class="textarea" id="com_size"                  name="com_size" 
			placeholder="输入公司规模"    ><% out.print(job_Info.getJASSET()); %></textarea>
		</label>
		
		<label class="label">
			<span class="label>span">公司实力：</span>
			<textarea class="textarea" id="company_strength"          name="com_strength" 
			placeholder="输入公司实力"    ><% out.print(job_Info.getJComStrength()); %></textarea>
		</label>
		<h5 class="h5">
		...
		<label class="label">
			...
			<input class="input[type=text]" id="contact_name"  type="text" name="contact_name" 
			placeholder="输入联系人姓名" value="<% out.print(job_Info.getLinkman()); %>"/>
		</label>
        <label class="label">
			<span class="label>span">联系电话：</span>
			<input class="input[type=text]" id="contact_phone" type="text" name="contact_phone" 
			placeholder="输入联系人电话" value="<% out.print(job_Info.getLinkTel()); %>"/>
		</label>
		<label class="label">
			<span class="label>span">联系邮箱：</span>
			<input class="input[type=text]" id="contact_email" type="text" name="contact_email"   
			placeholder="输入公司邮箱"  value="<% out.print(job_Info.getLinkEmail()); %>"/>
		</label>
		
		<label class="label">
			<span class="label>span">招聘期限：</span>
			<input class="input[type=text]" id="date"  type="text" name="date"   
			placeholder="填写招聘开始日期和截止日期" value="<% out.print(job_Info.getAddTime()); %>"/>
		</label>
		
		<label class="label">
			<span class="label>span">&nbsp;</span>
			<input class="button" type="submit" class="button" value="修改信息" />
		</label>
	</form>
	<!-- 修改招聘信息end -->
  </div>
